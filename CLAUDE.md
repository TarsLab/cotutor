...
- `agents/*.md` — **老师定义**(frontmatter 取两 CLI 公共子集:name / description / maxTurns / permissionMode / memory,正文即系统提示)。是出厂件:init **拷贝**进 workspace 的 `.claude/agents/`(2026-09-09 拍板,原为链),出厂 hash 记 `.cotutor/shipped.json`,`cotutor upgrade` 据此换新或报 diff(`src/cli/tutors.ts`);人设、政策、开关不在这里,在 workspace 的 cotutor.json
...
- **cotutor.json 在页面上能改**:title / policyDefaults / tutors / agents.default / paths / server / tts,补丁深合并到原始 JSON(保留 `$schema` `_note`),整份过契约才落盘,null 删键;kid / version / 预设模板走编辑器。老师文件正文走 `/api/tutors/<name>/file`(frontmatter name 必须不变),新老师 `POST /api/tutors` 与 `cotutor add` 同一条路,`DELETE` 只删自家的(文件改名保留)。服务每个请求看 mtime 热重载,改坏了留旧配置、`/api/health` 报 configError
