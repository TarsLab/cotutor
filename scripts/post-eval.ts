 ...
 *
 * 用法:node --experimental-strip-types scripts/post-eval.ts --workspace <dir> [--only <名>] [--repeat <n>] [--tutor <老师>] [--dialect json|html] [--template <file>] [--serial] [--model <名>]
 ...
 *   --dialect html 用代码里的 HTML 方言骨架(POST_TEMPLATE_HTML,2026-09-14 晚起也是出厂的)、json 用老的 JSON 骨架(POST_TEMPLATE_JSON),--template 指定一份文件;都不给就用 workspace 主题的
...
import { POST_TEMPLATE_JSON, dialectOf } from '../src/lib/postprocess.ts';
...
const dialectFlag = flag('dialect');
const templateFile = flag('template');
const serial = argv.includes('--serial');
const model = flag('model');
const template = templateFile ? readFileSync(templateFile, 'utf8') : dialectFlag === 'html' ? POST_TEMPLATE_HTML : dialectFlag === 'json' ? POST_TEMPLATE_JSON : undefined;
...
const dialectName = template ? dialectOf(template) : '主题的';
console.log(`后期评测:${names.length} 份样本 × ${repeat} 次 · 运行时 ${policy.post.runtime} · 主题 ${ws.config.kid.theme} · 骨架 ${templateFile ?? dialectName}${model ? ` · 模型 ${model}` : ''} · ${serial ? '各拍顺着起' : '各拍并行'} · 每拍超时 ${policy.post.timeoutMs}ms`);
...
for (const name of names) {
  ...
  for (let i = 0; i < repeat; i++) {
    ...
    writeFileSync(join(fx.dir, 'runs', `${stamp}-${policy.post.runtime}${model ? `-${model}` : ''}-${r.file.dialect ?? 'json'}${serial ? '-serial' : ''}${repeat > 1 ? `-${i + 1}` : ''}.json`), `${JSON.stringify({ at: new Date().toISOString(), runtime: policy.post.runtime, score, file: r.file, section: r.section }, null, 2)}\n`);
