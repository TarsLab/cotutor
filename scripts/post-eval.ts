 ...
 *
 * 用法:node --experimental-strip-types scripts/post-eval.ts --workspace <dir> [--only <名>] [--repeat <n>] [--tutor <老师>]
...
console.log(`后期评测:${names.length} 份样本 × ${repeat} 次 · 运行时 ${policy.post.runtime} · 主题 ${ws.config.kid.theme} · 每拍超时 ${policy.post.timeoutMs}ms`);
const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
const all: { name: string; score: FixtureScore; beatMs: number[] }[] = [];
for (const name of names) {
  const fx = loadPostFixture(join(root, name), name);
  for (let i = 0; i < repeat; i++) {
    const r = await runPost(ws, tutor, fx.section, { device: fx.expect.device, policy, env: process.env });
    ...
    writeFileSync(join(fx.dir, 'runs', `${stamp}-${policy.post.runtime}${repeat > 1 ? `-${i + 1}` : ''}.json`), `${JSON.stringify({ at: new Date().toISOString(), runtime: policy.post.runtime, score, file: r.file, section: r.section }, null, 2)}\n`);
