...
need(['text', 'read', 'choice', 'fill', 'image', 'tianzige', 'scene', 'canvas', 'code'].every((k) => existsSync(join(root, 'cards', k, 'card.md')) && existsSync(join(root, 'cards', k, 'card.css'))), 'cards/<kind>/ 不全(每种卡的 card.md + card.css)');
