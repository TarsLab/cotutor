...
- **老师看到的只有两样**:上下文包(run.json 的 prompt:老师守则(有脸的老师,话题第一条带 cotutor-tutor 技能原文)、当前学期、档案与入口文件的原文、参考路径、本周计划、最近 14 天日记的观察行、课程表时段、vault 路径、孩子上一轮在卡上做的、这条的照片)和它自己 Read 的文件(消息的 tools)。它看不到 cotutor.json、看不到别的老师、看不到今天之前的对话——只有 vault 里沉淀过的。
