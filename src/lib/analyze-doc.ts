...
上下文包里老师能看到的只有:profile(档案「现在」callout,按 profileLines 截)、plan(本周计划里这位老师的行,按 planLines 截)、recent(最近 14 天日记里本学科的「- 观察:」行,按 recent 取最新的)、slot(课程表命中的时段)、vault(路径)、cards(孩子上一轮在卡上做的)、photos(这条的作业照片)。别的它都看不到,除非自己 Read。
`;
}
