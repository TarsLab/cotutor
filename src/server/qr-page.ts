...
export function qrPage(title: string, info: ListenInfo, via: 'name' | 'ip' = 'name'): string {
  const useIp = via === 'ip' && info.ip !== null;
  const url = useIp ? (info.ip as string) : info.name;
  const svg = qrSvg(url) ?? '<p>地址太长,编不进二维码</p>';
  const other = useIp
    ? '<a href="/qr">换回主机名那张</a>(不随 Wi-Fi 变,适合打印)'
    : info.ip
      ? '扫完一直转圈、打不开?<a href="/qr?via=ip">换数字地址这张</a>'
      : '';
  return `<!doctype html>
<html lang="zh-CN">
<head>
