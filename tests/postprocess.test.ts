...

{
  ...
  check('出厂 post.md 与 POST_TEMPLATE_FALLBACK 同文;必需占位符齐;缺的点名', shipped === POST_TEMPLATE_FALLBACK && shipped.includes('{board}') && missingSlots(shipped).length === 0 && JSON.stringify(missingSlots('只有 {board}')) === '["rules","patch"]');
