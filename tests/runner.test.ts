 ...
 * 三轮 resume 同会话、换预设新开、跨天新开、忙时 409、出错标 error、待裁量物化、助教团补丁写回并热重载、关掉老师即消失。
 */
...
(cfg.teachers as Record<string, Record<string, unknown>>)['math-teacher'].voice = 'v-math';
(cfg.teachers as Record<string, Record<string, unknown>>)['reading-teacher'].voice = 'fail';
(cfg.teachers as Record<string, Record<string, unknown>>)['chinese-teacher'].policy = { dailyMessages: 1 };
...
writeFileSync(join(root, 'ledger', 'observations.jsonl'), '{"id":"o-1","date":"2026-09-06","author":"math-teacher","subject":"数学","claim":"借位忘了"}\n{"id":"o-2","date":"2026-09-07","author":"chinese-teacher","subject":"语文","claim":"错别字"}\n');

let now = new Date(2026, 8, 8, 16, 20);
const ctx = createContext(loadWorkspace(root), { now: () => now });
const post = (teacher: string, body: unknown) => route('POST', `/api/conversations/${teacher}/messages`, ctx, body);
const day = (teacher: string, date: string) => route('GET', `/api/conversations/${teacher}/${date}`, ctx);
...
const wait = async (teacher: string): Promise<void> => {
  // runner 没暴露 done 给路由层,这里轮询 running 直到空
  for (let i = 0; i < 200 && ctx.runner.running(teacher); i++) await new Promise((r) => setTimeout(r, 25));
};

try {
  // ---- 第一轮:新开 ----
  const r1 = await post('math-teacher', { text: '妈妈我不懂这一步', from: 'kid' });
  ...
  const busy = await post('math-teacher', { text: '再问' });
  check('老师还在回 → 409', busy.status === 409, JSON.stringify(busy.json));
  const mid = (await day('math-teacher', '2026-09-08')).json as Day;
  check('跑的时候索引已有 running 条目', mid.index.messages[0].result === 'running' && mid.running === job1);
  await wait('math-teacher');
  const d1 = (await day('math-teacher', '2026-09-08')).json as Day;
  ...
  const log = readFileSync(join(root, 'conversations', 'math-teacher', `2026-09-08.${job1}.log`), 'utf8');
  const init = JSON.parse(log.split('\n')[0]) as { cwd: string; agent: string };
  check('cwd 是老师目录、--agent 填了老师名', init.cwd === join(root, 'agents', 'math-teacher') && init.agent === 'math-teacher', JSON.stringify(init));
  check('err.log 落了', readFileSync(join(root, 'conversations', 'math-teacher', `2026-09-08.${job1}.err.log`), 'utf8').includes('fake-cli done'));
  ...
  const pack = await gatherContext(ctx.ws, 'math-teacher', { from: 'kid', at: now });
  ...
  const packZh = await gatherContext(ctx.ws, 'chinese-teacher', { from: 'parent', at: now });
  ...
  check('时段外没有 slot', (await gatherContext(ctx.ws, 'math-teacher', { from: 'kid', at: new Date(2026, 8, 8, 18, 0) })).slot === undefined);
  check('配音:老师配了 voice → mp3 落会话目录、索引带 audio', m1.audio === `2026-09-08.${job1}.mp3` && readFileSync(join(root, 'conversations', 'math-teacher', `2026-09-08.${job1}.mp3`), 'utf8').startsWith('fake-mp3:v-math:'), JSON.stringify(m1.audio));

  // ---- 第二、三轮:resume 同会话 ----
  now = new Date(2026, 8, 8, 16, 25);
  const r2 = await post('math-teacher', { text: '再讲一遍,要拍板', from: 'parent' });
  check('第二条 resume', (r2.json as { resume: boolean }).resume === true, JSON.stringify(r2.json));
  await wait('math-teacher');
  now = new Date(2026, 8, 8, 16, 30);
  await post('math-teacher', { text: '第三条,顺便转交' });
  await wait('math-teacher');
  const d3 = (await day('math-teacher', '2026-09-08')).json as Day;
  ...
  const dates = (await route('GET', '/api/conversations/math-teacher', ctx)).json as { dates: string[] };
  check('日期列表', dates.dates.join() === '2026-09-08');

  // ---- 换预设 → 新开,索引会话换掉 ----
  now = new Date(2026, 8, 8, 16, 40);
  const r4 = await post('math-teacher', { text: '换个 CLI', preset: 'fake2' });
  ...
  await wait('math-teacher');
  const d4 = (await day('math-teacher', '2026-09-08')).json as Day;
  check('换预设后会话换成新的', d4.index.session?.agent === 'fake2' && d4.index.session.id !== d1.index.session?.id);
  const log4 = readFileSync(join(root, 'conversations', 'math-teacher', '2026-09-08.1640-4.log'), 'utf8');
  check('{agentBody} 给了老师正文', (JSON.parse(log4.split('\n')[0]) as { bodyLen: number }).bodyLen > 100);
  const bad = await post('math-teacher', { text: 'x', preset: 'gemini' });
  check('不存在的预设 → 400', bad.status === 400 && String((bad.json as { message: string }).message).includes('gemini'));

  // ---- 跨天新开 ----
  now = new Date(2026, 8, 9, 8, 0);
  const r5 = await post('math-teacher', { text: '新的一天' });
  check('零点后第一条不 resume、落新文件', (r5.json as { resume: boolean; date: string }).resume === false && (r5.json as { date: string }).date === '2026-09-09');
  await wait('math-teacher');
  check('两天两份索引', existsSync(join(root, 'conversations', 'math-teacher', '2026-09-09.json')) && existsSync(join(root, 'conversations', 'math-teacher', '2026-09-08.json')));
  check('today 别名', ((await day('math-teacher', 'today')).json as Day).index !== undefined);

  // ---- 出错:CLI 报 error / 起不来 ----
  await post('reading-teacher', { text: '会失败', preset: 'broken' });
  await wait('reading-teacher');
  const dr = (await day('reading-teacher', '2026-09-09')).json as Day;
  ...
  await post('reading-teacher', { text: '配音会失败' });
  await wait('reading-teacher');
  const dr2 = (await day('reading-teacher', '2026-09-09')).json as Day & { index: { messages: { audio?: string | null }[] } };
  check('配音失败 → audio null、对话照常、原因进 err.log', dr2.index.messages[1].result === 'ok' && dr2.index.messages[1].audio === null && readFileSync(join(root, 'conversations', 'reading-teacher', `2026-09-09.${dr2.index.messages[1].job}.err.log`), 'utf8').includes('没合成'), JSON.stringify(dr2.index.messages[1]));
  await post('chinese-teacher', { text: '起不来', preset: 'missing' });
  await wait('chinese-teacher');
  const dm = (await day('chinese-teacher', '2026-09-09')).json as Day;
  ...
  const home = (await route('GET', '/api/kid/home', ctx)).json as { title: string; timetable: unknown[]; teachers: { name: string; available: boolean; remaining: number; hasVoice: boolean }[]; stacks: unknown[] };
  check('首页:标题、课程表、孩子端老师(无 planner)', home.title === '小明的书房' && home.timetable.length === 2 && home.teachers.length === 4 && !home.teachers.some((t) => t.name === 'planner') && home.stacks.length === 0, JSON.stringify(home.teachers));
  check('老师带 hasVoice 与剩余条数(今天 09-09 孩子还没发过)', home.teachers.find((t) => t.name === 'math-teacher')?.hasVoice === true && home.teachers.find((t) => t.name === 'math-teacher')?.remaining === 30, JSON.stringify(home.teachers));
  const kd = (await route('GET', '/api/kid/conversations/math-teacher/today', ctx)).json as { messages: { question: string | null; reply: string | null; audio: string | null }[]; remaining: number; pending: string | null };
  ...
  const kp = await route('POST', '/api/kid/conversations/math-teacher/messages', ctx, { text: '孩子问的' });
  check('孩子发消息 202', kp.status === 202, JSON.stringify(kp.json));
  const kdMid = (await route('GET', '/api/kid/conversations/math-teacher/today', ctx)).json as { pending: string | null; messages: { pending: boolean }[] };
  check('跑的时候 pending', kdMid.pending !== null && kdMid.messages[1].pending === true);
  await wait('math-teacher');
  const kd2 = (await route('GET', '/api/kid/conversations/math-teacher/today', ctx)).json as { messages: { question: string | null; reply: string | null; audio: string | null }[] };
  ...
  const au = await route('GET', '/api/audio/math-teacher/2026-09-09.0800-2.mp3', ctx);
  ...
  check('配音路径校验', (await route('GET', '/api/audio/math-teacher/../cotutor.json', ctx)).status === 404 && (await route('GET', '/api/audio/math-teacher/2026-09-09.0800-9.mp3', ctx)).status === 404);
  // 语文老师 dailyMessages = 1:孩子发一条后头像灰、再发 429;家长发的不算
  check('语文老师上限前可用', (await route('GET', '/api/kid/home', ctx)).status === 200 && ((await route('GET', '/api/kid/home', ctx)).json as typeof home).teachers.find((t) => t.name === 'chinese-teacher')?.available === true);
  check('孩子发第一条 202', (await route('POST', '/api/kid/conversations/chinese-teacher/messages', ctx, { text: '一' })).status === 202);
  await wait('chinese-teacher');
  const limited = await route('POST', '/api/kid/conversations/chinese-teacher/messages', ctx, { text: '二' });
  check('到上限 → 429,头像灰', limited.status === 429 && ((await route('GET', '/api/kid/home', ctx)).json as typeof home).teachers.find((t) => t.name === 'chinese-teacher')?.available === false, JSON.stringify(limited.json));
  check('家长照发不受孩子上限', (await post('chinese-teacher', { text: '家长的', from: 'parent' })).status === 202);
  await wait('chinese-teacher');
  const kdZh = (await route('GET', '/api/kid/conversations/chinese-teacher/today', ctx)).json as { messages: { question: string | null }[]; remaining: number };
  check('家长那条的问句不给孩子看,remaining 0', kdZh.messages.filter((m) => m.question !== null).length === 1 && kdZh.remaining === 0, JSON.stringify(kdZh));
  check('孩子端页面在,没有「错误」字样', ((await route('GET', '/', ctx)).html ?? '').includes('小明的书房') && !((await route('GET', '/', ctx)).html ?? '').includes('错误'));

  // ---- 参数校验 ----
  check('空消息 400', (await post('math-teacher', { text: '   ' })).status === 400);
  check('坏 from 400', (await post('math-teacher', { text: 'x', from: 'dog' })).status === 400);
  check('没这位老师 404', (await post('nobody', { text: 'x' })).status === 404);
  check('坏日期 400', (await day('math-teacher', '2026-9-9')).status === 400);

  // ---- 助教团:补丁写回、热重载、关掉老师即消失 ----
  const p1 = await route('PATCH', '/api/config', ctx, { teachers: { 'math-teacher': { enabled: false, policy: { replyMaxChars: 20 } } } });
  check('PATCH 写回并热重载', p1.status === 200 && ctx.ws.config.teachers['math-teacher'].enabled === false, JSON.stringify(p1.json));
  const raw = JSON.parse(readFileSync(cfgFile, 'utf8')) as { teachers: Record<string, { enabled: boolean; display: string; policy: { replyMaxChars: number } }>; _note?: string; agents: { default: string } };
  check('文件里只动了那几个字段,_note 与预设都在', raw.teachers['math-teacher'].enabled === false && raw.teachers['math-teacher'].display === '数学老师' && raw.teachers['math-teacher'].policy.replyMaxChars === 20 && typeof raw._note === 'string' && raw.agents.default === 'fake');
  const teachersNow = (await route('GET', '/api/config', ctx)).json as { teachers: { name: string; enabled: boolean; policy: { replyMaxChars: number } }[] };
  check('接口回报关闭与有效政策', teachersNow.teachers.find((t) => t.name === 'math-teacher')?.enabled === false && teachersNow.teachers.find((t) => t.name === 'math-teacher')?.policy.replyMaxChars === 20);
  check('孩子端列表里没了', !((await route('GET', '/api/teachers?kid=1', ctx)).json as { name: string }[]).some((t) => t.name === 'math-teacher'));
  check('关掉的老师不收消息', (await post('math-teacher', { text: 'x' })).status === 400);
  ...
  check('家长页在', ((await route('GET', '/parent', ctx)).html ?? '').includes('助教团'));
} finally {
