/** 最终文本里的「## 待裁量」「## 记账」段:剥出来、正文干净;解析不出整段留正文;「## 转交」已不是固定段。 */
...

{
  ...
  check('正文去掉待裁量段、保留其它段', p.body.startsWith('我看了照片') && p.body.includes('## 备注') && p.body.endsWith('两道题都是退位没借。') && !p.body.includes('to:') && !p.body.includes('question:'), p.body);
}
{
  ...
  check('代码块里的 ## 不算标题', fence.holdup === null && fence.body.includes('## 待裁量'));
  ...
  check('没有段', plain.holdup === null && plain.bookkeeping === null && plain.body === '就一句话。');
}
done();
